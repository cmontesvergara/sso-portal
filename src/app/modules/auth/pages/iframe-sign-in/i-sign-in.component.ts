...
@Component({
  selector: 'app-i-sign-in',
  templateUrl: './i-sign-in.component.html',
  styleUrls: ['./i-sign-in.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    ReactiveFormsModule,
...
export class IFrameSignInComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      // Fetch dynamic tenant metadata
      if (this.tenantId) {
        this.isFetchingTenant = true;
        this.authService.getTenantPublicInfo(this.tenantId).subscribe({
          next: (res) => {
            ...
            this.isFetchingTenant = false;
          },
          error: (err) => {
            console.error('[SignIn] Failed to fetch tenant public info:', err);
            this.isTenantValid = false;
            this.isFetchingTenant = false;
          }
        });
    ...
    // Setup postMessage listener and emit sso-ready
  ...
  private setupPostMessageListener() {
    window.addEventListener('message', (event: MessageEvent) => {
      ...
      if (msg.type === 'sso-init' && msg.v === '2.3') {
        ...
        // Extract sanitized hostname from origin (strip protocol)
        if (msg.payload?.origin) {
          try {
            this.iframeOrigin = new URL(msg.payload.origin).hostname;
          } catch {
            this.iframeOrigin = msg.payload.origin.replace(/^https?:\/\//, '');
          }
        }
        this.appName = this.getAppName(this.appId);
        console.log(
          `[SignIn] Forced app-initiated mode. redirectUri: ${this.redirectUri}, appId: ${this.appId}, tenantId: ${this.pkceContext.tenantId}, iframeOrigin: ${this.iframeOrigin}`
        );
      }
    });
  }
